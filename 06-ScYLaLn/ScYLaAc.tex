...
\frame{
	...
	\begin{itemize}
		...
		\item Aktinium je přítomno pouze ve stopovém množství v uranových rudách, kde vzniká radioaktivním rozpadem.
...
\frame{
	...
	\vfill
	\begin{itemize}
		\item \textit{Bastnezity} je skupina minerálů s fluorid-uhličitanovým aniotem.\footnote[frame]{\href{https://www.mindat.org/min-563.html}{Bastnäsite-(Ce)}}
...
\frame{
	...
	\begin{itemize}
		...
		\item Využívá se také pro kontinuální měření tloušťky plastů, pryže a papíru.\footnote[frame]{\href{https://www.radiacode.com/eur/isotope/pm-147}{Promethium-147}}
	\end{itemize}
	\vfill
}
...
\frame{
	...
	\begin{columns}
		\begin{column}{.6\textwidth}
			\begin{itemize}
				\item \textbf{Thorium} se získává loužením monazitového písku v 73\% NaOH při teplotě 140~$^\circ$C.
...
\frame{
	...
	\begin{columns}
		\begin{column}{.65\textwidth}
			\begin{itemize}
				\item \textbf{Plutonium} bylo poprvé připraveno bombardováním $^{238}$U jádry deuteria v cyklotronu.\footnote[frame]{\href{https://discover.lanl.gov/publications/national-security-science/2021-winter/plutonium-timeline/}{Atomic number 94}}
				\item \ce{^{238}_{\ \ 92}U + ^2_1H -> ^{238}_{\ \ 93}Np + 2 ^1_0n}
				\item \ce{^{238}_{\ \ 93}Np -> ^{238}_{\ \ 94}Pu + ^0_{-1}$\beta$^-}
				\item Nejvýznamnějším izotopem je štěpitelný $^{239}$Pu, ten se připravuje dlouhodobým ozařováním uranu neutrony v reaktoru.
				\item \ce{^{238}_{\ \ 92}U + ^1_0n -> ^{239}_{\ \ 92}U + $\gamma$}
				\item \ce{^{239}_{\ \ 92}U -> ^{239}_{\ \ 94}Pu + 2 ^0_{-1}$\beta$^-}
...
\frame{
	\frametitle{}
	\begin{itemize}
		\item \textbf{Skandium} nemá příliš velké využití.
...
\frame{
	...
	\begin{figure}
		...
		\caption*{Krystalová struktura YBCO.\footnote[frame]{Zdroj: \href{https://commons.wikimedia.org/wiki/File:Ybco002.svg}{Rswarbrick/Commons}}}
	\end{figure}
		\end{column}
	\end{columns}
}
	...
	\begin{columns}
		\begin{column}{.6\textwidth}
			\begin{itemize}
				...
				\item Tyto supravodiče mají vysokou hodnotu kritické teploty i kritického pole, proto jsou to velmi perspektivní materiály pro vývoj reaktorů pro jadernou fúzi.\footnote[frame]{\href{https://tokamakenergy.com/2023/02/06/world-first-super-magnets-built-by-tokamak-energy-for-fusion-power-plant-testing/}{World-first supermagnets built by Tokamak Energy for fusion power plant testing}}
				\item Využívají se i v nově budovaném stelarátoru firmy Proxima Fusion.\footnote[frame]{\href{https://www.innovationnewsnetwork.com/proxima-fusion-unveils-breakthrough-concept-for-commercial-fusion-power-plant/55878/}{Proxima Fusion unveils breakthrough concept for commercial fusion power plant}}$^,$\footnote[frame]{\href{https://www.osel.cz/13930-jaderna-fuze-proxima-fusion-stellaris-fuzni-elektrarna-se-stelaratorem.html}{Jaderná fúze: Proxima Fusion Stellaris – fúzní elektrárna se stelarátorem}}
			\end{itemize}
		\end{column}
		\begin{column}{.45\textwidth}
			\begin{figure}
				\adjincludegraphics[width=\textwidth]{img/NbTi3SnWire.jpg}
	...
	\begin{columns}
		\begin{column}{.7\textwidth}
			\begin{itemize}
				...
				\item Očekávaný termín spuštění je v roce 2033, plného výkonu by měl dosáhnout o pět let později.
			\end{itemize}
		\end{column}
...
\frame{
	...
	\begin{itemize}
		...
		\item Na rozdíl od bomb je nutné vyvarovat se řetězovému průběhu jaderné reakce, proto reaktory obsahují prvky, které snižují počet volných neutronů.
	\end{itemize}
	\vfill
}
...
\frame{
	...
	\begin{itemize}
		\item Technologický proces vyvinutý během projektu Manhattan k získávání plutonia a uranu z vyhořelého jaderného paliva.\footnote[frame]{\href{https://web.archive.org/web/20181123200013/http://www.igcar.gov.in/rpg/articles/Ramanujam\%20-\%20Intro\%20to\%20reporcessing.pdf}{An Introduction to the Purex Process}}
...
\frame{
	...
	\begin{itemize}
		\item Přírodní jaderný reaktor v Oklo, ve státě Gabon na západním pobřeží Afriky.\footnote[frame]{\href{https://www.iaea.org/newscenter/news/meet-oklo-the-earths-two-billion-year-old-only-known-natural-nuclear-reactor}{Meet Oklo, the Earth’s Two-billion-year-old only Known Natural Nuclear Reactor}}
...
\frame{
	...
	\begin{block}{Oxid skanditý}
		\begin{itemize}
			\item Oxid skanditý vyniká vysokou teplotní odolností.
			\item Nový žáruvzdorný materiál vyvinutý v NASA vznikl při experimentálním studiu interakce mezi taveninou simulovaného lunárního regolitu a vybranými oxidickými přísadami. Klíčovým krokem syntézy bylo vysokoteplotní zpracování směsi obsahující oxid skanditý a simulovaný měsíční regolit, které vedlo k vzniku dosud nepopsané fáze.\footnote[frame]{\href{https://www.nasa.gov/general/new-material-melt-moon-rocks/}{New Material Could Help NASA Melt Moon Rocks, Harness Lunar Resources}}
			\item Nový materiál je levnější než tradičně používané materiály (např. platina) a zároveň lehčí a lépe izoluje teplo. Mohl by být použitelný pro konstrukci zařízení na zpracování měsíčních surovin (např. potrubí nebo nádoby na roztavený materiál).\footnote[frame]{\href{https://kosmonautix.cz/2026/05/28/nasa-testuje-material-ktery-muze-pomoci-ziskavat-zdroje-na-mesici/}{NASA testuje materiál, který může pomoci získávat zdroje na Měsíci}}
		\end{itemize}
	\end{block}
	\vfill
}
	...
	\begin{columns}
		\begin{column}{.7\textwidth}
			\begin{itemize}
				...
				\item Můžeme jej připravit fluorací dusičnanu nebo oxidu lanthanitého:
				\end{itemize}
				\begin{align*}
				\ce{La(NO3)3 + 3 HF &-> LaF3 + 3 HNO3}\\
				\ce{La2O3 + 6 NH4HF2 &-> 2 LaF3 + 6 NH4F + 3 H2O}
				\end{align*}
				\begin{itemize}
...
\frame{
	...
	\begin{columns}
		\begin{column}{.7\textwidth}
			\begin{itemize}
				\item \textbf{Amidy lanthanoidů} patří mezi důležité sloučeniny, protože jsou zpravidla rozpustné a lze je poměrně snadno získat v krystalické podobě.
				\item Připravují se např. reakcí chloridu lanthanoidy s trimethylsilylamidem lithným:\footnote[frame]{\href{https://doi.org/10.1039/DT9730001021}{The preparation and characterization of tris{bis(trimethylsilyl)-amido}lanthanides}}
			\end{itemize}
		\end{column}
		\begin{column}{.3\textwidth}
			\begin{figure}
				\adjincludegraphics[width=.8\textwidth]{img/MN-tms.png}
				\caption*{Obecný vzorec trimethylsilylamidu kovu.}
			\end{figure}
		\end{column}
	\end{columns}
	\begin{Center}
		\ce{PrCl3 + 3 Li[N(SiMe3)2] ->[THF] Pr[N(SiMe3)2]3 + 3 LiCl}
	\end{Center}
	\vfill
}
