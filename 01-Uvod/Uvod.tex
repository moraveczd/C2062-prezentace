...
\frame{
	...
	\begin{itemize}
		...
		\item Informace k organizaci přednášky a průběhu zkoušky najdete v interaktivní osnově v ISu.
	\end{itemize}
	\vfill
}
...
\frame{
	...
	\begin{itemize}
		\item Aktuální verze všech prezentací jsou dostupné v interaktivní osnově a také na adrese:
		\begin{itemize}
...
\frame{
	...
	\begin{enumerate}
		...
		\href{https://is.muni.cz/do/rect/el/estud/pedf/js18/obecna_chemie/web/index.html}{Obecná chemie} -- online
		\item \href{https://cs.wikibooks.org/wiki/Anorganick\%C3\%A1_chemie}{Anorganická chemie} -- online, zatím ve výstavbě, můžete se zapojit
	\end{enumerate}
}
...
\frame{
	...
	\textbf{Slitiny}
	\begin{itemize}
		\item Materiály tvořené dvěma či více prvky, z nichž alespoň jeden je kov.
...
\frame{
	\frametitle{}
	\vfill
	\begin{alertblock}{Eutektikum}
		Tuhá směs dvou látek s minimem teploty tání.
	\end{alertblock}
...
\frame{
	...
	\begin{columns}
		\begin{column}{0.6\textwidth}
			\vfill
			\begin{alertblock}{Intermetalikum}
...
\frame{
	\frametitle{}
	\vfill
	\begin{alertblock}{Ocel}
		Slitina železa s uhlíkem a dalšími prvky, která obsahuje \textit{méně než 2,14~\% uhlíku}.
	\end{alertblock}
	...
	\begin{itemize}
		...
		\item Surové železo se získává ve vysokých pecích redukcí železné rudy (\ce{Fe3O4}, \ce{Fe2O3}) koksem v přítomnosti struskotvorné přísady.
	\end{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		\item Jde např. o \ce{Fe3O4} nebo \ce{NiFe2O4}.
...
\frame{
	...
	\vfill
	\begin{itemize}
		\item \textit{Altermagnetické látky} jsou kombinací feromagnetických a antiferomagnetických látek.
		...
		\item Altermagnety nevykazují makroskopickou magnetizaci jako feromagnety, ale přesto vykazují spinové rozštěpení elektronových pásů.
	\end{itemize}
	\vfill
}
...
\frame{
	...
	\begin{itemize}
		...
		\item Prvky jedné periody mají stejnou nejvyšší obsazenou hlavní kvantovou hladinu.
		\item Všechny dosud známé prvky jsou v periodách 1---7.
...
\frame{
	...
	\begin{alertblock}{d-orbitaly}
		zaplňují se až po zaplnění s-orbitalu s hlavním kvantovým číslem (n+1), např. 3d orbital se začne plnit až po 4s
	\end{alertblock}
...
\frame{
	\frametitle{}
	\textbf{Příklady elektronových konfigurací}

	\begin{tabular}{ll||ll}
		Rb & [Kr] 5s$^1$ & Ti & [Ar] 3d$^{2}$ 4s$^2$ \\
		Sr & [Kr] 5s$^2$ & Ti$^{3+}$ & [Ar] 3d$^{1}$ 4s$^0$ \\
		Sr$^{2+}$ & [Kr] 5s$^0$ & Ti$^{4+}$ & [Ar] 3d$^{0}$ 4s$^0$ \\
		Al & [Ne] 3s$^2$ 3p$^1$ & Cr & [Ar] 3d$^{5}$ 4s$^1$ \\
		Ga & [Ar] 3d$^{10}$ 4s$^2$ 4p$^1$ & Fe & [Ar] 3d$^{6}$ 4s$^2$ \\
		Ga$^{+}$ & [Ar] 3d$^{10}$ 4s$^2$ 4p$^0$ & Fe$^{2+}$ & [Ar] 3d$^{6}$ 4s$^0$ \\
		Ga$^{3+}$ & [Ar] 3d$^{10}$ 4s$^0$ 4p$^0$ & Fe$^{3+}$ & [Ar] 3d$^{5}$ 4s$^0$ \\
		Bi & [Xe] 4f$^{14}$ 5d$^{10}$ 6s$^2$ 6p$^3$ & Cu & [Ar] 3d$^{10}$ 4s$^1$ \\
		Bi$^{3+}$ & [Xe] 4f$^{14}$ 5d$^{10}$ 6s$^2$ 6p$^0$ & La & [Xe] 5d$^{1}$ 6s$^2$ \\
		Bi$^{5+}$ & [Xe] 4f$^{14}$ 5d$^{10}$ 6s$^0$ 6p$^0$ & Ce & [Xe] 4f$^{1}$ 5d$^{1}$ 6s$^2$ \\
		Po & [Xe] 4f$^{14}$ 5d$^{10}$ 6s$^2$ 6p$^4$ & Dy & [Xe] 4f$^{10}$ 6s$^2$ \\
	\end{tabular}
}
...
\frame{
	\frametitle{}
	\vfill
	\begin{alertblock}{Lanthanoidová kontrakce}
	Se stoupajícím protonovým číslem lanthanoidů dochází ke zmenšování iontového poloměru.
	\end{alertblock}
...
\frame{
	...
	\vfill
	\begin{itemize}
		\item \textit{Elektronová afinita} je změna energie při přijetí elektronu atomem v plynném stavu.
		\item \ce{A (g) + e^- -> A^- (g)}
...
\frame{
	\frametitle{}
\begin{alertblock}{Koordinační sloučeniny}
	Obsahují alespoň jednu koordinační (donor-akceptorovou) vazbu mezi centrálním kovem a~ligandem.
\end{alertblock}

	\begin{columns}
		\begin{column}{.85\textwidth}
			\begin{itemize}
				\item \textbf{Koordinační vazba} je dvouelektronová chemická vazba, kde oba elektrony pocházejí z~jednoho atomu (\emph{donoru}), druhý atom (\emph{akceptor}) poskytuje pro tyto elektrony volný orbital.
...
\frame{
	...
	\begin{tabular}{lll}
		...
		\ce{F^{-}} & Fluorid & Fluorido- \\
		\ce{O^{2-}} & Oxid & Oxido- \\
		\ce{H^{-}} & Hydrid & Hydrido- \\
		\ce{CN^{-}} & Kyanid & Kyanido- \\
		\ce{SCN^{-}} & Thiokyanatan & Thiokyanato- \\
	\end{tabular}
	\vspace{5mm}
	\\
	\begin{tabular}{ll}
		\ce{K3[Fe(CN)6]} & hexakyanidoželezitan draselný \\
		\ce{K4[Fe(CN)6]} & hexakyanidoželeznatan draselný \\
		\ce{Na3[CrF6]} & hexafluoridochromitan sodný \\
	\end{tabular}
}
...
\frame{
	\frametitle{}
	\vfill
	\begin{alertblock}{Chelátový efekt}
		Komplexy s vícedentátními ligandy jsou řádově stabilnější než komplexy s monodentátními ligandy.
	\end{alertblock}

	\begin{itemize}
		\item Tyto komplexy se označují jako \textit{chelátové}, z řeckého chelos -- klepeto.\footnote[frame]{\href{http://doi.wiley.com/10.1002/hlca.19520350721}{Der Chelateffekt}}
		...
		\item \ce{[M(H2O)6]^{2+} + H2EDTA^{2-} <=> [M(EDTA)]^{2-} + 2 H+ + 6 H2O}
	\end{itemize}
	\vfill
}
...
\frame{
	\frametitle{}
		...
		\begin{figure}
			...
			\caption*{Iont kovu chelatovaný EDTA}
		\end{figure}
		\vfill
}
...
\frame{
	\frametitle{}
	\begin{alertblock}{Jahnův-Tellerův teorém}
		{\large \uv{Každá nelineární molekula, která má elektrony v degenerovaném stavu, bude nestálá a bude se deformovat tak, aby vznikl systém o nižší energii, v němž bude degenerace odstraněna.}}
	\end{alertblock}
	\begin{itemize}
...
\frame{
	\frametitle{}
	\begin{itemize}
		\item U oktaedrických komplexů může dojít buď k jejich protažení nebo zkrácení.
...
\frame{
	\frametitle{}
	Supertěžké prvky mají protonové číslo vyšší než 103. Uvedeny jsou poločasy nejdéle žijících nebo nejlépe charakterizovaných izotopů.
...
\frame{
	...
	\begin{itemize}
		...
		\item První ostrov stability se předpokládá u neutronového čísla 184, které dle výpočtů patří mezi nejslibnější neutronová magická čísla.
		\item Jedním z kandidátů je jádro $^{298}_{114}$Fl. Příprava těchto jader je ovšem velmi komplikovaná, např.:
